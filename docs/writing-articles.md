...
Individual articles with at least two main headings have a sticky contents rail when the available container is at least 60rem wide. It starts beside the article's breadcrumb and header, then stays below the sticky site header with a 1rem gap while reading. Every page shares the same header and divider width, capped at the full three-column article width, regardless of its contents or margin notes. Heading and footnote jumps clear the measured header height. Smaller layouts retain the normal site header, horizontal progress bar, and collapsible contents. The rail highlights the current heading and reveals that section's level-three headings; its vertical track shows approximate reading position.
